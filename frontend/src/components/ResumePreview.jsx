  ...
  const renderCreativeSlideTemplate = () => {
    return (
      <div className="resume-a4-page animate-fade-in" style={{
        ...
        {/* Header Section */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #f1f5f9', paddingBottom: '1.5rem' }}>
          <div>
            <h1 style={{ 
              fontSize: '2.5rem', 
            ...
            {/* Contact details */}
            <div style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap', fontSize: '0.85rem', color: '#555' }}>
        ...
        {personal.summary && (
          <div>
            <h2 style={{ fontSize: '1.25rem', color: 'var(--color-primary)', fontFamily: 'var(--font-serif)', borderBottom: '1px solid #f1f5f9', paddingBottom: '0.35rem', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Professional Profile
            </h2>
            <p style={{ fontSize: '0.95rem', lineHeight: '1.6', color: '#4a5568', textAlign: 'justify' }}>
              {personal.summary}
            </p>
          </div>
        )}
        ...
        {experience.length > 0 && (
          <div>
            <h2 style={{ fontSize: '1.25rem', color: 'var(--color-primary)', fontFamily: 'var(--font-serif)', borderBottom: '1px solid #f1f5f9', paddingBottom: '0.35rem', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Work Experience
            </h2>
              ...
              {experience.map((exp, idx) => (
                <div key={idx}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.25rem' }}>
                    <h3 style={{ fontSize: '1.05rem', color: '#1a202c', fontWeight: 700 }}>
                  ...
                  {exp.location && <p style={{ fontSize: '0.8rem', color: '#718096', fontStyle: 'italic', marginBottom: '0.35rem' }}>{exp.location}</p>}
                  <p style={{ fontSize: '0.9rem', lineHeight: '1.5', color: '#4a5568', whiteSpace: 'pre-line' }}>
                    {exp.description}
                  </p>
                </div>
              ))}
              ...
              {projects.map((proj, idx) => (
                <div key={idx} style={{ border: '1px solid #e2e8f0', borderRadius: 'var(--radius-sm)', padding: '1rem', background: '#fafbfc' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
                    <h3 style={{ fontSize: '0.95rem', color: 'var(--color-primary)', fontWeight: 700 }}>{proj.title}</h3>
                    {proj.link && (
                  ...
                  {proj.role && <p style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-secondary)', marginBottom: '0.5rem' }}>{proj.role}</p>}
                  <p style={{ fontSize: '0.8rem', color: '#4a5568', lineHeight: '1.4', marginBottom: '0.5rem' }}>{proj.description}</p>
                  {proj.technologies && (
                    <p style={{ fontSize: '0.75rem', color: '#718096', borderTop: '1px dashed #e2e8f0', paddingTop: '0.35rem', marginTop: '0.35rem' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {education.map((edu, idx) => (
                    <div key={idx}>
                      <h4 style={{ fontSize: '0.95rem', color: '#1a202c', fontWeight: 700 }}>{edu.degree}</h4>
                      <p style={{ fontSize: '0.85rem', color: 'var(--color-secondary)', fontWeight: 600 }}>{edu.school}</p>
                      <p style={{ fontSize: '0.75rem', color: '#718096' }}>{edu.gradDate} {edu.score && `| Grade: ${edu.score}`}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {certifications.map((cert, idx) => (
                    <div key={idx} style={{ display: 'flex', gap: '0.4rem', alignItems: 'flex-start' }}>
                      ...
                      <div>
                        <h4 style={{ fontSize: '0.85rem', color: '#1a202c', fontWeight: 700, margin: 0 }}>{cert.title}</h4>
                        <p style={{ fontSize: '0.75rem', color: '#718096', margin: 0 }}>{cert.issuer} {cert.date && `(${cert.date})`}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '2rem', marginTop: 'auto', borderTop: '2px solid #f1f5f9', paddingTop: '1.5rem' }}>
          {/* Skills */}
          <div>
            <h2 style={{ fontSize: '1.1rem', color: 'var(--color-primary)', fontFamily: 'var(--font-serif)', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              Skills & Expertise
            </h2>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {personal.website && (
                <a href={personal.website} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem', color: 'var(--color-secondary)', fontWeight: 600 }}>
                  <Globe size={14} />
                  <span>Personal Website / Portfolio</span>
                </a>
              )}
              {links.map((link, idx) => (
                <a key={idx} href={link.url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem', color: 'var(--color-primary)', fontWeight: 600 }}>
